...
params.scale_sigma_factor = 1/4;        % standard deviation for the desired scale filter output超参数
params.lambda = 1e-2;					% regularization weight (denoted "lambda" in the paper)
params.learning_rate = 0.025;			% tracking model learning rate (denoted "eta" in the paper)学习域
params.number_of_scales = 33;           % number of scale levels (denoted "S" in the paper)33个尺度空间
params.scale_step = 1.02;               % Scale increment factor (denoted "a" in the paper)
params.scale_model_max_area = 512;      % the maximum size of scale examples最大区域放大空间

params.visualization = 1;

%ask the user for the video加载视频
